...
class Vue():
    ...
    def creerFrameCasUsage(self):
        ...
        self.listeCasUsage.bind("<ButtonRelease>", self.afficherScenario)
        self.listeCasUsage.pack(side=LEFT, expand=1,fill=BOTH)
        
        self.scrollCasUsage = Scrollbar(self.listeCas)
        self.scrollCasUsage.pack(side=LEFT, expand=1,fill=BOTH)
    ...
    def inscrireCasUsageEntryDansListe(self, cas):
        texteCas=cas.casUsage
        self.listeCasUsage.insert(END, texteCas)       
        
        
    def inscrireLigneUsager(self):
        texteLigne = self.entryUsager.get()
        cas=self.entryCas.get()
        self.parent.inscrireLigne("usager", texteLigne, cas)
        self.listeUsager.insert(END, texteLigne)
    ...
    def inscrireLigneOrdi(self):
        texteLigne = self.entryOrdi.get()
        cas=self.entryCas.get()
        self.parent.inscrireLigne("ordi", texteLigne, cas)
        self.listeOrdi.insert(END, texteLigne)
        self.listeUsager.insert(END, "")
        self.listeAutre.insert(END, "")
        self.entryOrdi.delete(0, END)
        
    def inscrireLigneAutre(self):
        texteLigne = self.entryAutre.get()
        cas=self.entryCas.get()
        self.parent.inscrireLigne("autre", texteLigne, cas)
        self.listeAutre.insert(END, texteLigne)
        self.listeOrdi.insert(END, "")
        self.listeUsager.insert(END, "")
    ...
    def afficherScenario(self, evt):
        self.root.update_idletasks()
        
        index=self.listeCasUsage.curselection()
        cas=self.listeCasUsage.get(index)
        rep=self.parent.chercherCas(cas)
        
        self.effacerListBox()
        self.entryCas.delete(0, END)

        self.entryCas.insert(0, rep.casUsage)

        for i in rep.scenarioUsager:
            self.listeUsager.insert(END,i)
    
        for i in rep.scenarioOrdi:
            self.listeOrdi.insert(END,i)
            
        for i in rep.scenarioAutre:
            self.listeAutre.insert(END,i)
