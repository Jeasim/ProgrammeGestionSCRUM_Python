...
class Controleur():
    ...
    def creerCas(self, cas):
        ...
        self.serveur.entreeGenerique1(requete)
    ...
    def commitLigne(self, resp, texte, cas):
        ...
        self.serveur.entreeGenerique1(requete)
        print(self.serveur.entreeGenerique1("SELECT * FROM ligne_cas WHERE id_cas = '"+str(rep.idBD)+"';"))
    ...
    def uploadBD(self, idProjet):
        ...
        if nbCas != "no such column: id_projet":
            for i in range(nbCas[0][0]):
                ...
                nbLignes=self.serveur.entreeGenerique1("SELECT COUNT(*) FROM ligne_cas WHERE id_cas = '"+str(rep[i][0])+"';")
                lignes=self.serveur.entreeGenerique1("SELECT * FROM ligne_cas WHERE id_cas = '"+str(rep[i][0])+"';")
                print(lignes)
                
                for j in range(nbLignes[0][0]):
                    casTexte=self.serveur.entreeGenerique1("SELECT description FROM cas_usage WHERE id = '"+str(lignes[j][1])+"';")
                    print(casTexte)
